...
export default function GitHubTab() {
  ...
  const handleTestConnection = async () => {
    if (!connection?.user) {
      setConnectionTest({ status: 'error', message: 'No connection established', timestamp: Date.now() });
      return;
    }
    ...
    try {
      ...
      setConnectionTest(
        isValid
          ? { status: 'success', message: `Connected successfully as ${connection.user.login}`, timestamp: Date.now() }
          : { status: 'error', message: 'Connection test failed', timestamp: Date.now() },
      );
    } catch (error) {
      setConnectionTest({
        status: 'error',
    ...
    return (
      <div className="min-w-0 space-y-6">
        <div className="flex min-w-0 items-center gap-2">
          <GithubLogo />
          <h2 className="min-w-0 text-lg font-medium text-bolt-elements-textPrimary">GitHub Integration</h2>
        </div>
  ...
  if (error && !connection) {
    return (
      <div className="min-w-0 space-y-6">
        ...
        <ErrorState title="Connection Error" message={error} onRetry={() => window.location.reload()} retryLabel="Reload Page" />
      </div>
    );
  }
  ...
  if (!isConnected || !connection) {
    return (
      <div className="min-w-0 space-y-6">
        ...
        <p className="text-sm leading-6 text-bolt-elements-textSecondary">
          Connect your GitHub account to enable repository management, statistics, and project import.
        </p>
    ...
    <GitHubErrorBoundary>
      <div className="min-w-0 space-y-6 overflow-x-hidden">
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            <GithubLogo />
            <h2 className="min-w-0 truncate text-lg font-medium text-bolt-elements-textPrimary">GitHub Integration</h2>
          </div>
          {connection?.rateLimit && (
            <div className="flex w-fit max-w-full items-center gap-2 rounded-lg bg-bolt-elements-background-depth-1 px-3 py-1 text-xs">
              <div className="i-ph:cloud h-4 w-4 shrink-0 text-bolt-elements-textSecondary" />
        ...
        <p className="text-sm leading-6 text-bolt-elements-textSecondary">
          Manage your GitHub integration with repository features and connection statistics.
        </p>
          ...
          >
            <Collapsible open={isReposExpanded} onOpenChange={setIsReposExpanded}>
              <CollapsibleTrigger asChild>
                <button className="flex min-h-12 w-full min-w-0 items-center justify-between gap-3 rounded-lg border border-bolt-elements-borderColor bg-bolt-elements-background p-4 text-left transition-all duration-200 hover:border-bolt-elements-borderColorActive/70 dark:bg-bolt-elements-background-depth-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <div className="i-ph:folder h-4 w-4 shrink-0 text-bolt-elements-item-contentAccent" />
                    <span className="min-w-0 truncate text-sm font-medium text-bolt-elements-textPrimary">
                      All Repositories ({stats.repos.length})
                    </span>
                  </div>
                  <ChevronDown
                    className={classNames(
                      'h-4 w-4 shrink-0 transform text-bolt-elements-textSecondary transition-transform duration-200',
                ...
                <div className="mt-4 min-w-0 space-y-4">
                  <div className="grid min-w-0 grid-cols-1 gap-4 md:grid-cols-2">
                    {(isReposExpanded ? stats.repos : stats.repos.slice(0, 12)).map((repo) => (
                      <div key={repo.full_name} className="min-w-0">
                        <RepositoryCard
                          repository={repo}
                          variant="detailed"
                          showHealthScore
                  ...
                  {stats.repos.length > 12 && !isReposExpanded && (
                    <div className="text-center">
                      <Button variant="outline" onClick={() => setIsReposExpanded(true)} className="w-full sm:w-auto">
        ...
        {statsError && !stats && (
          <ErrorState title="Failed to Load Statistics" message={statsError} onRetry={() => window.location.reload()} retryLabel="Retry" />
        )}
        ...
        {isConnected && connection && (
          <div className="min-w-0 border-t border-bolt-elements-borderColor pt-6 sm:mt-8">
